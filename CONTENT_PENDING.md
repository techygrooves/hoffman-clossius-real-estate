...
| 4.9 | Accessibility statement contact route | 🟢 | **Resolved.** The client has confirmed Martin's direct line (954-605-4823) may be published as the accessibility contact, and `/accessibility/` publishes it alongside both email addresses and the contact form. The statement describes WCAG 2.2 Level AA as the **target** and **claims no certification or conformance** — see `ACCESSIBILITY_AUDIT.md`. Still open if the client later wants a dedicated accessibility inbox or a named owner other than Martin. |
| 4.9a | Independent accessibility audit | 🟠 | The site has been audited in-house (`ACCESSIBILITY_AUDIT.md`: axe-core clean across both build modes, plus keyboard, focus and reflow checks). It has **not** been audited by an accessibility specialist, and **not** tested with people who use assistive technology. `/accessibility/` says so in public. Commissioning either is a client decision with a cost attached. |
